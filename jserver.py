...
description = """Whisper JAX is an optimised implementation of the [Whisper model](https://huggingface.co/openai/whisper-small.en) by OpenAI. It runs on JAX with our laptop GPU in the backend. Compared to PyTorch on an A100 GPU, it is still probably faster, making it the fastest Whisper API available.
...
article = "Whisper small.en model by OpenAI. Backend running JAX on a GPU, thanks to the generous support of the community. Whisper JAX [code](https://github.com/sanchit-gandhi/whisper-jax) and Gradio demo by 🤗 Hugging Face."
