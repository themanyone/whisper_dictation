...
def initialize_project(q=None):
    """Scan the project and write/update AGENTS.md."""
    agents_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                               "AGENTS.md")
    root = os.path.dirname(os.path.abspath(__file__))
    if _scan_project(root=root, agents_path=agents_path):
        say("Initialized project file.")
    else:
        say("Project file is already up to date.")
            ...
            messages=[
                {
                    ...
                    "content": (
                        ...
                        '"argument": null}\n\n'
                        "The optional 'argument' field controls argument extraction:\n"
                        '- null (default): no argument, shell command is literal\n'
                        '- "remainder": everything said after the intent phrase is\n'
                        "  extracted and substituted into {q} in the shell command\n\n"
                        ...
                        '"argument": null}\n'
                        '"search the web for italian restaurants"\n'
                        '  → {"action": true, "intent": "search the web", '
                        '"shell": "firefox \\\\"https://duckduckgo.com/?q={q}\\\\"", '
                        '"desc": "Search the web for {q}", '
                        '"argument": "remainder"}\n'
                        '"how are you today"\n'
                        '  → {"action": false}\n\n'
                        ...
                        "the appropriate intent, shell command, and argument type."
                    ),
                },
...
def generate_text(prompt: str):
    ...
    try:
        ...
        if completion and any(sig in completion for sig in
                             ("dalle.text2im", "image_gen")):
            try:
                j = json.loads(completion)
                if j.get("action") in ("dalle.text2im", "image_gen"):
...
register_handler("image_gen", _img_tool)
register_handler("dalle.text2im", _img_tool)
            ...
            if f := audio_queue.get():
                ...
                if not chatting and has_wake:
                    ...
                    if proposal:
                        ...
                        if not shell:
                            continue
                        if proposal.get("argument") == "remainder":
                            handler_code = (
                        ...
                        if response == "yes":
                            ...
                            if save_resp == "yes":
                                save_custom_command(
                                    ...
                                    handler_code, proposal.get("argument"),
                                )
                                matcher.add_command(
                                    proposal["intent"], handler_name,
